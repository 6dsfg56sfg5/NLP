...
class NeuralNetwork:
    def __init__(self, input_size: int, hidden_neurons: int):
        ...
        self.weights_input_hidden = np.random.randn(input_size, hidden_neurons) * 0.1
    ...
    def train(self, X: np.ndarray, y: np.ndarray, epochs: int = 1000, lr: float = 0.01, log_interval: int = 100):
        y = y.reshape(-1, 1)
...
if __name__ == "__main__":
    ...
    np.savetxt('weights_input_hidden.csv', nn.weights_input_hidden, delimiter=',')
    np.savetxt('weights_hidden_output.csv', nn.weights_hidden_output, delimiter=',')
